...
import {
    ...
    loadPatientData, bookAppointment, findPatientAppointments, cancelAppointmentByDetails, rescheduleAppointmentByDetails
} from "./appointment.js";
import { processPrescriptionRefill } from './prescriptionRefill.js';
import Appointment from "../models/Appointment.js";
import { AudioBufferManager, transcribeAudio } from "./transcription.js";
            ...
            switch (functionName) {
                ...
                case 'book_appointment':
                    ...
                    if (result.success) {
                        ...
                        // Legacy: Update MongoDB if patientId exists
                        if (patientId) {
                            upcomingAppointments = await getUpcomingAppointments(patientId);
                        }
                    }
                ...
                case 'check_patient_exists':
                    ...
                    console.log('ðŸ” Checking if patient exists in EMR');
                ...
                case 'update_patient_info':
                    ...
                    console.log('Updating patient info in EMR');

                    // For EMR-only, we need the patient's FHIR ID
                    // If we have patientData with fhirId, use it
                    if (patientData?.fhirId) {
                        result = await updatePatientInfo(patientData.fhirId, args);
                        extractedEntities = { ...extractedEntities, patient_info: args };
                    } else if (patientId) {
                        // Legacy: If we only have MongoDB ID, try to find FHIR ID
                        // This is a fallback for backwards compatibility
                        try {
                            const patient = await Patient.findById(patientId);
                            if (patient?.fhirId) {
                                result = await updatePatientInfo(patient.fhirId, args);
                                extractedEntities = { ...extractedEntities, patient_info: args };
                            } else {
                                result = {
                                    success: false,
                                    message: "Patient FHIR ID not found. Cannot update in EMR."
                                };
                            }
                        } catch (error) {
                            result = {
